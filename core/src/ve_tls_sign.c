    ...
    if (headers_in && headers_in[0] != 0) {
        ...
        while (*hp) {
            ...
            if (!skip) {
                ...
                if (line_len > 0) {
                    /* 复用 ve_tls_buf_append 的小缓冲区路径：直接逐字符 append 不安全，
                     * 改为先复制到临时缓冲。考虑到行长有限，使用栈缓冲；超长则放弃过滤逐行追加。 */
                    char line_buf[1024];
