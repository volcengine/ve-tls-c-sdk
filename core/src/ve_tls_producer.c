...
ve_tls_producer * ve_tls_producer_create(const ve_tls_config * config) {
    ...
    producer->config = effective;
    producer->cfg_endpoint = ve_tls_strdup(effective.endpoint);
    producer->cfg_region = ve_tls_strdup(effective.region);
    producer->cfg_project_id = ve_tls_dup_cstr(effective.project_id);
    producer->cfg_topic_id = ve_tls_strdup(effective.topic_id);
    producer->cfg_source = ve_tls_dup_cstr(effective.source);
    producer->cfg_file_name = ve_tls_dup_cstr(effective.file_name);
    producer->cfg_context_flow = ve_tls_dup_cstr(effective.context_flow);
    producer->cfg_hash_key = ve_tls_dup_cstr(effective.hash_key);
    producer->cfg_access_key_id = effective.access_key_id ? ve_tls_strdup(effective.access_key_id) : NULL;
    producer->cfg_access_key_secret = effective.access_key_secret ? ve_tls_strdup(effective.access_key_secret) : NULL;
    producer->cfg_security_token = (effective.security_token && effective.security_token[0] != 0) ? ve_tls_strdup(effective.security_token) : NULL;
    producer->cfg_api_version = ve_tls_dup_cstr(effective.api_version);
    producer->cfg_compress_type = ve_tls_dup_cstr(effective.compress_type);
    producer->cfg_ca_cert_path = ve_tls_dup_cstr(effective.ca_cert_path);
    producer->cfg_proxy = ve_tls_dup_cstr(effective.proxy);
    producer->cfg_user_agent = ve_tls_dup_cstr(effective.user_agent);
    if (effective.log_tag_count > 0 && !effective.log_tags) {
        ve_tls_producer_destroy(producer);
        return NULL;
    }
    if (ve_tls_copy_log_tags(effective.log_tags, effective.log_tag_count, &producer->cfg_log_tags, &producer->cfg_log_tag_count) != 0) {
        ve_tls_producer_destroy(producer);
        return NULL;
    }
...
static ve_tls_result ve_tls_producer_add_log_kv_lens_time_parts_hashkey(
    ...
    if (flush) {
        ...
        int should_seal = 0;
        if (producer->config.log_count_per_package > 0 && b->log_count >= (size_t)producer->config.log_count_per_package) {
            should_seal = 1;
        }
        if (!should_seal && byte_limit > 0 && b->logs_len >= byte_limit) {
            should_seal = 1;
