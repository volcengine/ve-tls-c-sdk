                ...
                while (stack && stack_len > 0) {
                    ...
                    if (max_comp > 0) {
                        ...
                        if (scratch_rc != 0) {
                            ...
                            err.transport_code = 0;
                            err.retryable = 0;
                            err.error_code = ve_tls_strdup((scratch_rc == -2) ? "ProducerClosed" : (scratch_rc == -3) ? "BufferFullTimeout" : "BufferFull");
                            err.error_message = ve_tls_strdup("buffer budget exceeded before compression");
                            ve_tls_send_callbacks cbs = ve_tls_capture_callbacks(producer);
                            if (cbs.cb) {
                    ...
                    int reserve_rc = ve_tls_producer_reserve_send_task_bytes(producer, &t);
                    int push_rc = reserve_rc == 0 ? ve_tls_send_queue_push(&producer->send_queue, &t, wait_ms) : reserve_rc;
                    if (push_rc != 0) {
                        if (reserve_rc == 0) {
                            ve_tls_producer_release_send_queue_task_bytes(producer, &t);
                        }
                        ...
                        if (push_rc == -2) {
                            ...
                            err.error_code = ve_tls_strdup("BufferFullTimeout");
                            err.error_message = ve_tls_strdup("buffer budget wait timeout");
                        } else if (reserve_rc != 0) {
                            err.error_code = ve_tls_strdup("BufferFull");
                            err.error_message = ve_tls_strdup("buffer budget exceeded");
                        } else {
                            int stopped = 0;
                            if (producer->send_queue.platform && producer->send_queue.mutex) {
                                producer->send_queue.platform->mutex_lock(producer->send_queue.mutex);
