...
BUILD_DIR="${BUILD_DIR:-$ROOT_DIR/build_tls_perf}"
...
WRITERS="${WRITERS:-1}"
PROFILES="${PROFILES:-tls700 tls200}"
...
cmake --build "$BUILD_DIR" -j --target ve_tls_perf_tls >/dev/null
...
for P in $PROFILES; do
  for RATE in $RATE_LIST; do
    ...
    OUT="$("$BUILD_DIR/ve_tls_perf_tls" "${ARGS[@]}")"
