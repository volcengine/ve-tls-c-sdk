...
#define RUN(code, fn) do { if ((fn) != 0) { rc = (code); goto end; } } while (0)
    ...
    RUN(141, test_raw_add_log_budget_full_drops_before_copy_alloc());
    RUN(142, test_kv_add_log_budget_full_drops_before_builder_grow());
    RUN(143, test_breaker_ingress_fail_fast_rejects_before_queue());
    RUN(144, test_breaker_ingress_drop_with_callback_reports_drop_without_queue());
    RUN(145, test_scratch_budget_is_counted_against_max_buffer_bytes());
